...
class UsbCanTranslator(CanTranslator):
    ...
    def writecanfile(self, fileName):
        try:
            data = open(fileName, "r")
        except IOError as e:
            print "I/O error({0}): {1}".format(e.errno, e.strerror)
        else:
            outgoingBytearray = bytearray()
            messageCount = 0
            for line in data:
                timeStamp, _, lastBit = line.partition(" ")
                while timeStamp == "":   #Remove the leading spaces.
                    timeStamp, _, lastBit = lastBit.partition(" ")
            
                channelNumber, _, lastBit = lastBit.partition("  ")  #Extra space after the channel number
                if channelNumber is not "1":
                    continue
                messageID, _, lastBit = lastBit.partition(" ")

                thisID = int(messageID, 16)

                if messageID == "430":
                    print thisID

                _, _, lastBit = lastBit.partition("Rx   d 8 ")
                data, _, _ = lastBit.partition("\r")
                
                dataArray = bytearray.fromhex(data)
                dataArray.insert(0, '{')  # 7B == '{'
                dataArray.insert(1, (thisID & 0xff000000) >> 24)
                dataArray.insert(2, (thisID & 0x00ff0000) >> 16)
                dataArray.insert(3, (thisID & 0x0000ff00) >> 8)
                dataArray.insert(4, (thisID & 0x000000ff))
                dataArray.insert(5, '|')  # 0x7C == '|'
                dataArray.extend('}')  # 7D == '}'
                dataArray.extend("\0")

                outgoingBytearray.extend(dataArray)
                messageCount += 1

                if(messageCount >= 1):
                    self.out_endpoint.write(dataArray)  #TODO: determine if we're going for speed.  If yes, combine messages into 64 byte chunks.
                    messageCount = 0
                    outgoingBytearray = bytearray()
...
def parse_options():
    ...
    parser.add_argument("--writefile",
            ...
            dest="writefile",
            default="")
    parser.add_argument("--writecanfile",
            action="store",
            dest="writecanfile",
            default="")
...
def main():
    ...
    elif arguments.writecanfile is not "":
        device.writecanfile(arguments.writecanfile)
    else:
        device.run()

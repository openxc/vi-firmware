        ...
        case INITIALIZE:
            ...
            switch(subState) {
                ...
                case 2:
                    ...
                    memcpy(device->deviceId, IMEI, strlen(IMEI) < MAX_DEVICE_ID_LENGTH ? 
                        strlen(IMEI) : MAX_DEVICE_ID_LENGTH);
                    ...
                    if(setNetworkConnectionMode(device->config.networkOperatorSettings.operatorSelectMode, 
                        device->config.networkOperatorSettings.networkDescriptor) == false) {
                        subState = 0;
                        state = POWER_OFF;
                        break;
                    }
            ...
            switch(subState) {
                ...
                case 1:
                    ...
                    if(getNetworkConnectionStatus(&connectStatus)) {
                        if(connectStatus == REGISTERED_HOME || 
                            (device->config.networkOperatorSettings.allowDataRoaming && connectStatus == 
                            REGISTERED_ROAMING)) {
                            if(getCurrentNetwork(&current_network)) {
                                debug("Telit connected to PLMN %u, access type %u", 
                                    current_network.PLMN, current_network.networkType);
                            }
                            subState = 0;
                            state = CLOSE_PDP;
                        } else {
        ...
        case READY:
            ...
            switch(subState) {
                case 0:
                    ...
                    if(getNetworkConnectionStatus(&connectStatus), (connectStatus != REGISTERED_HOME && 
                        connectStatus != REGISTERED_ROAMING)) {
    ...
    switch(state) {
        ...
        case 0:
            ...
            sprintf(header, "POST /api/%s/data HTTP/1.1\r\n"
                    ...
                    "Content-Type: %s\r\n"
                    "Host: %s\r\n"
                    "Connection: Keep-Alive\r\n\r\n", deviceId, len, getConfiguration()->payloadFormat == 
                        PayloadFormat::PROTOBUF ? ctPROTOBUF : ctJSON, host);
